...
sys.path.append("/scratch/ja961/evopro/")
...
sys.path.append('/scratch/ja961/alphafold/run/')
...
def run_af2_dist(args, score_func=None):
    ...
    # Load sequences
    seqs_list = []
    with open('protease_out/'+args.output+'/'+args.sequence_file, "r") as sf:
        for line in sf:
    ...
    if args.max_chains_length:
        ...
        lengths = [[len(x) for x in seqs_list[0][0][0].split(",")]]
    ...
    for seq,i in zip(sorted_seqs_and_scores, range(len(sorted_seqs_and_scores))):
        ...
        with open('protease_out/'+args.output+'/seq_'+str(i)+'_model_1.pdb','w') as pdbf:
            pdbf.write(str(pdb))
    ...
    with open('protease_out/'+args.output+'/seqs_and_scores.csv','w') as opf:
        for seq,i in zip(sorted_seqs_and_scores, range(len(sorted_seqs_and_scores))):
